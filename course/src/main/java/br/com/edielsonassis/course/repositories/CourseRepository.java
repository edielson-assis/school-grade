...
public interface CourseRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
    ...
    @Modifying
    @Query(value = "delete from course_user where course_id = :courseId", nativeQuery = true)
    void deleteCourseUserByCourse(@Param("courseId") UUID courseId);
	
    @Modifying
    @Query(value = "delete from course_user where user_id = :userId", nativeQuery = true)
